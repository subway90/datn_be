cls
php artisan migrate:refresh --seed
php artisan db:seed --class=TienIchToaNhaSeeder
php artisan db:seed --class=KhuVucSeeder
php artisan db:seed --class=ToaNhaSeeder
php artisan db:seed --class=PhongSeeder
php artisan db:seed --class=HopDongSeeder
php artisan db:seed --class=HoaDonSeeder
php artisan db:seed --class=DanhMucTinTucSeeder
php artisan db:seed --class=TinTucSeeder
php artisan db:seed --class=BinhLuanTinTucSeeder
php artisan db:seed --class=LienHeDatPhongSeeder
php artisan db:seed --class=BinhLuanToaNhaSeeder
php artisan db:seed --class=BannerSeeder
php artisan db:seed --class=DangKyNhanTInSeeder
php artisan db:seed --class=ConfigWebsiteSeeder